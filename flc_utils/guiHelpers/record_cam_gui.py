...
configparser.read('flc_utils/screens/mohaliGPU/gui.cfg')

if not os.path.exists("flc_utils/trainVideo/training/"):
	os.mkdir("flc_utils/trainVideo/training/")

last_video = ""
writer = cv2.VideoWriter(__file__.replace("flc_utils/guiHelpers/record_cam_gui.py", "flc_utils/trainVideo/training/cam_record.mp4"), cv2.VideoWriter_fourcc(*'MP4V'), int(configparser.get('gui-config', 'fps')), (int(configparser.get('gui-config', 'recording_resolution_width')), int(configparser.get('gui-config', 'recording_resolution_height'))), True)

window = tk.Tk()  #Makes main window
window.wm_title("Recording...")
#window.geometry("%dx%d+%d+%d" % (600, 500, 50, 20))
...
def save_and_close():
	...
	if name in ["Enter video name..", "Please enter video name", ""]:
		...
		last_video = name
		os.rename("flc_utils/trainVideo/training/cam_record.mp4", "flc_utils/trainVideo/training/" + "_".join(str(datetime.datetime.now())[:-7].split()) + "_" + name + ".mp4")
		window.destroy()

def discard_video():
	if last_video == "":
		cmd = "rm -rf flc_utils/trainVideo/training/cam_record.mp4"
	else:
		cmd = "rm -rf flc_utils/trainVideo/training/" + last_video + ".mp4"
	os.system(cmd)
	window.destroy()
...
#Graphics window
imageFrame = tk.Frame(window, width=400, height=350)
