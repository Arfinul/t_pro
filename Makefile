...
CC=gcc
CPP=g++
NVCC=/usr/local/cuda-9.0/bin/nvcc # GCP PATH
# NVCC=nvcc # KGP GPU PATH
OPTS=-Ofast
LDFLAGS= -lm -pthread 
COMMON= 
...
ifeq ($(GPU), 1)
COMMON+= -DGPU -I/usr/local/cuda-9.0/include/ # # GCP PATH
# COMMON+= -DGPU -I/usr/local/cuda/include/ # KGP GPU PATH
CFLAGS+= -DGPU
ifeq ($(OS),Darwin) #MAC
LDFLAGS+= -L/usr/local/cuda-9.0/lib -lcuda -lcudart -lcublas -lcurand # # GCP PATH
# LDFLAGS+= -L/usr/local/cuda/lib -lcuda -lcudart -lcublas -lcurand # KGP GPU PATH
else
LDFLAGS+= -L/usr/local/cuda-9.0/lib64 -lcuda -lcudart -lcublas -lcurand # GCP PATH
# LDFLAGS+= -L/usr/local/cuda/lib64 -lcuda -lcudart -lcublas -lcurand # KGP GPU PATH
endif
endif
...
COMMON+= -DCUDNN
ifeq ($(OS),Darwin) #MAC
CFLAGS+= -DCUDNN -I/usr/local/cuda-9.0/include # GCP PATH
# CFLAGS+= -DCUDNN -I/usr/local/cuda/include # KGP GPU PATH
LDFLAGS+= -L/usr/local/cuda-9.0/lib -lcudnn # GCP PATH
# LDFLAGS+= -L/usr/local/cuda/lib -lcudnn # KGP GPU PATH
else
