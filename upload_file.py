...
s3_client = boto3.client('s3', aws_access_key_id='AKIAZ47ERKTBHM6BHB5R', region_name='ap-south-1', aws_secret_access_key='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')
...
def fileOps():
    ...
    if (get_size()/(1024*1024)) > MAX_SIZE:
        ...
        if no_of_files > 0:
            ...
            if 'result.mp4' in list_of_files:
                list_of_files.remove('result.mp4')
...
def uploadVidsToS3(file_to_upload):
    ...
    s3_client.upload_file(
            file, 
            S3_BUCKET, 'aideobarie_xavier_nx_2/{}'.format(key),
