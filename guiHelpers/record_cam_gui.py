...
configparser.read('screens/mohaliGPU/gui.cfg')

if not os.path.exists("trainVideo/training/"):
	os.mkdir("trainVideo/training/")

last_video = ""
writer = cv2.VideoWriter(__file__.replace("guiHelpers/record_cam_gui.py", "trainVideo/training/cam_record.mp4"), cv2.VideoWriter_fourcc(*'MP4V'), int(configparser.get('gui-config', 'fps')), (int(configparser.get('gui-config', 'recording_resolution_width')), int(configparser.get('gui-config', 'recording_resolution_height'))), True)
...
def save_and_close():
	...
	if name in ["Enter video name..", "Please enter video name", ""]:
		...
		last_video = name
		os.rename("trainVideo/training/cam_record.mp4", "trainVideo/training/" + "_".join(str(datetime.datetime.now())[:-7].split()) + "_" + name + ".mp4")
		window.destroy()

def discard_video():
	if last_video == "":
		cmd = "rm -rf trainVideo/training/cam_record.mp4"
	else:
		cmd = "rm -rf trainVideo/training/" + last_video + ".mp4"
	os.system(cmd)
	window.destroy()
